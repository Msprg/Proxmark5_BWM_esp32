...
// Power-save switch (app_power.c), persisted in NVS. Default on.
static uint8_t g_power_save = 1;
...
static void app_nvs_flash_load(void) {
    ...
    err = settings_power_save_load(&g_power_save, g_power_save);
    if (err == ESP_OK) {
        g_power_save = (g_power_save != 0) ? 1 : 0;
        ESP_LOGI(TAG, "Loaded power save from NVS: %u", (unsigned)g_power_save);
    }
...
void app_main(void) {
    ...
    // Power-save switch: DFS + light sleep + low-duty advertising, or none of
    // them (persisted, APP_CMD_SET_SYS_POWER_SAVE). Before the BLE stack starts
    // so the first advertising run already uses the right interval. The UART
    // link keeps its own no-light-sleep lock while there is traffic (app_cmd_uart.c).
    ESP_ERROR_CHECK_WITHOUT_ABORT(app_power_init(g_power_save != 0));
