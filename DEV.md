...
A host that never sends the preamble still works, except that the first frame after
a quiet spell is lost; retry it. DFS (`CONFIG_PM_ENABLE`) runs independently of this.

**Power-save switch.** Everything above, DFS and the 3 s advertising interval
...
> **Source**: `components/app_uart_cmd/app_cmd_uart.c` (`link_touch`, `link_sleep_init`), `components/app_power/app_power.c`
